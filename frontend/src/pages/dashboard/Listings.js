...
const Listings = () => {
  ...
  const handleFileSelect = (e) => {
    ...
    if (files.length > maxImages) {
      alert(`You can only select ${maxImages} more images`);
      return;
    }
  ...
  const handleUpload = async () => {
    try {
      setUploading(true);
      const result = await uploadImages(selectedFiles, formData.id || 'temp');
      setFormData({
        ...formData,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (modalMode === 'add') {
        ...
        const result = await listingService.updateListing(preparedData.id, preparedData);
        if (result) {
  ...
  useEffect(() => {
    ...
    fetchListings();
  }, [searchTerm, categoryFilter, subcategoryFilter, statusFilter, isFeaturedFilter, priceFilter, page, pageSize]);
  ...
  useEffect(() => {
    fetchCategories();
    fetchListings();
  }, [page, pageSize, searchTerm, categoryFilter, subcategoryFilter, statusFilter, isFeaturedFilter, priceFilter]);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {listings.length > 0 ? (
              listings.map(listing => (
                <div key={listing.id} className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200 hover:shadow-lg transition-shadow">
                  ...
                  <div className="p-4">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className="text-primary-600 font-bold">
                        {formatPrice(listing.price)}/{listing.price_period || 'day'}
                      </span>
                    </div>
              ...
              <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <form onSubmit={handleSubmit}>
                  <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div className="mb-4">
                      <h3 className="text-lg font-medium text-gray-900">{modalMode === 'add' ? 'Add New Listing' : 'Edit Listing'}</h3>
                    </div>
                      ...
                      <div className="flex flex-col space-y-4">
                        {formData.images && formData.images.length > 0 ? (
                          <div className="grid grid-cols-5 gap-4 mb-2">
                            ...
                            {Array.from({ length: 5 - (formData.images?.length || 0) }).map((_, index) => (
                              <div key={`empty-${index}`} className="h-24 w-full rounded-md border border-dashed border-gray-300 flex items-center justify-center bg-gray-50">
                        ...
                        <div className="flex items-center">
                          <div className="flex-1 relative">
                            ...
                            <label 
                              ...
                              className="w-full px-4 py-2 border border-gray-300 rounded-md cursor-pointer hover:bg-gray-50 transition-colors"
                            >
                          ...
                          <button
                            ...
                            disabled={selectedFiles.length === 0 || uploading || (formData.images && formData.images.length >= 5)}
                          >
                            {uploading ? 'Uploading...' : 'Upload'}
                          </button>
                        </div>
                        <p className="text-xs text-gray-500">
                          {formData.images ? `${formData.images.length}/5 images added` : '0/5 images added'}
                        </p>
                      </div>
                    </div>
              ...
              <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  <div className="mb-4 flex justify-between items-center">
                    ...
                    <button onClick={closeViewModal} className="text-gray-400 hover:text-gray-700">
                      <RiCloseLine className="w-6 h-6" />
                    </button>
                  </div>
                  <div className="mb-4 overflow-y-auto max-h-[70vh]">
                    <div className="font-bold text-xl mb-1">{viewListing.title}</div>
                    <div className="text-gray-600 mb-2">{viewListing.description}</div>
                    ...
                    <div className="mb-4">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div className="text-sm">
                          <span className="font-medium text-gray-700">Category:</span> {viewListing.category?.name || viewListing.category}
                        </div>
                        <div className="text-sm">
                          <span className="font-medium text-gray-700">Subcategory:</span> {viewListing.subcategory?.name || viewListing.subcategory}
                        </div>
                        <div className="text-sm">
                          <span className="font-medium text-gray-700">Brand:</span> {viewListing.brand || 'N/A'}
                        </div>
                        <div className="text-sm">
                          <span className="font-medium text-gray-700">Condition:</span> {viewListing.condition || 'N/A'}
                        </div>
                        <div className="text-sm">
                          <span className="font-medium text-gray-700">Status:</span> {viewListing.status}
                        </div>
                        <div className="text-sm">
                          <span className="font-medium text-gray-700">Featured:</span> {viewListing.is_featured ? 'Yes' : 'No'}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mb-4">
                      <h4 className="font-semibold text-gray-800 mb-2">Pricing & Availability</h4>
                      <div className="grid grid-cols-2 gap-2">
                        <div className="text-sm">
                          <span className="font-medium text-gray-700">Price:</span> <span className="font-semibold text-primary-700">${viewListing.price}</span>
                        </div>
                        <div className="text-sm">
                          <span className="font-medium text-gray-700">Period:</span> {viewListing.price_period || 'Per Day'}
                        </div>
                        <div className="text-sm">
                          <span className="font-medium text-gray-700">Deposit:</span> ${viewListing.deposit || '0'}
                        </div>
                        <div className="text-sm">
                          <span className="font-medium text-gray-700">Min Duration:</span> {viewListing.min_duration || '1'} {viewListing.price_period || 'day(s)'}
                        </div>
                        <div className="text-sm">
                          <span className="font-medium text-gray-700">Available From:</span> {viewListing.available_from ? new Date(viewListing.available_from).toLocaleDateString() : 'N/A'}
                        </div>
                        <div className="text-sm">
                          <span className="font-medium text-gray-700">Available To:</span> {viewListing.available_to ? new Date(viewListing.available_to).toLocaleDateString() : 'N/A'}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mb-4">
                      <h4 className="font-semibold text-gray-800 mb-2">Location & Delivery</h4>
                      <div className="grid grid-cols-2 gap-2">
                        <div className="text-sm">
                          <span className="font-medium text-gray-700">Location:</span> {viewListing.location}
                        </div>
                        <div className="text-sm">
                          <span className="font-medium text-gray-700">Delivery:</span> {viewListing.delivery ? 'Available' : 'Not Available'}
                        </div>
                        {viewListing.delivery && (
                          <div className="text-sm">
                            <span className="font-medium text-gray-700">Shipping Fee:</span> ${viewListing.shipping || '0'}
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    {viewListing.specifications && viewListing.specifications.length > 0 && (
                      <div className="mb-4">
                        <h4 className="font-semibold text-gray-800 mb-2">Specifications</h4>
                        <div className="bg-gray-50 p-2 rounded">
                          {viewListing.specifications.map((spec, idx) => (
                            <div key={idx} className="text-sm flex justify-between py-1 border-b border-gray-100 last:border-0">
                    ...
                    <div className="mb-4">
                      <h4 className="font-semibold text-gray-800 mb-2">Terms & Policies</h4>
                      <div className="bg-gray-50 p-2 rounded">
                        {viewListing.rental_terms && (
                          <div className="text-sm mb-2">
                            <span className="font-medium text-gray-700 block mb-1">Rental Terms:</span>
                            <p className="text-gray-600">{viewListing.rental_terms}</p>
                          </div>
                        )}
                        {viewListing.cancellation && (
                          <div className="text-sm mb-2">
                            <span className="font-medium text-gray-700 block mb-1">Cancellation Policy:</span>
                            <p className="text-gray-600">
                              {viewListing.cancellation === 'flexible' && 'Flexible - Full refund 1 day prior'}
                              {viewListing.cancellation === 'moderate' && 'Moderate - Full refund 3 days prior'}
                              {viewListing.cancellation === 'strict' && 'Strict - 50% refund up to 1 week'}
                              {viewListing.cancellation === 'non_refundable' && 'Non-refundable'}
                            </p>
                          </div>
                        )}
                        {viewListing.notes && (
                          <div className="text-sm">
                            <span className="font-medium text-gray-700 block mb-1">Additional Notes:</span>
                        ...
                        <div className="grid grid-cols-2 gap-2 mt-2">
                          {viewListing.images.map((img, idx) => (
                            <img
                              ...
                              onError={(e) => { e.target.src = PLACEHOLDER_IMAGE; }}
                              className="rounded shadow border object-cover w-full h-32"
                            />
                          ))}
                        </div>
                      ) : (
                    ...
                    {viewListing.video && (
                      <div className="mb-4">
                        ...
                        <div className="text-sm">
                          <a href={viewListing.video} target="_blank" rel="noopener noreferrer" className="text-primary-600 hover:underline">
                            Watch Video
                          </a>
                        </div>
                      </div>
                    ...
                    <div className="mt-4 text-xs text-gray-500">
                      <div>Created: {new Date(viewListing.created_at).toLocaleString()}</div>
                      <div>Last Updated: {new Date(viewListing.updated_at).toLocaleString()}</div>
                    </div>
                  </div>
                </div>
              </div>
