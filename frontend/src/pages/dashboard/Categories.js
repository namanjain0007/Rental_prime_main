...
// Function to get icons from localStorage
const getCategoryIcons = () => {
  try {
    const storedIcons = localStorage.getItem('categoryIcons');
    return storedIcons ? JSON.parse(storedIcons) : {};
  } catch (error) {
    console.error('Error getting icons from localStorage:', error);
    return {};
  }
};

// Function to save icons to localStorage
const saveCategoryIcons = (icons) => {
  try {
...
const Categories = () => {
  ...
  // Store icons separately keyed by category ID
  const [categoryIcons, setCategoryIcons] = useState(() => {
    const icons = getCategoryIcons();
    console.log('Initial category icons from localStorage:', icons);
    return icons;
  });
  ...
  }, [searchTerm, viewMode, categoryIcons]);
      ...
      <div key={category.id} className="mb-3">
        <div className="flex items-center p-2 hover:bg-gray-50 rounded-md border border-gray-100 shadow-sm hover:shadow-md transition-all">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center mt-1 space-x-2">
              ...
              <span className={`px-2 py-0.5 text-xs rounded-full ${category.status === 'active' ? 'bg-green-50 text-green-700' : 'bg-yellow-50 text-yellow-700'}`}>
                {category.status}
              </span>
            </div>
          </div>
        ...
        {category.children && category.children.length > 0 && (
          <div className="pl-6 border-l-2 border-blue-200 ml-5 mt-2">
            <div className="text-xs text-gray-500 mb-2 ml-2">{category.children.length} Subcategories</div>
  ...
  const fetchCategoryTree = async () => {
    try {
      ...
      console.log('Current icons in localStorage:', categoryIcons);
  ...
  const processCategoryTreeIcons = (categories) => {
    return categories.map(category => {
      // Check if we have a saved icon for this category
      const savedIconData = categoryIcons[category.id];
      
      // Default icon and type
      let icon = '📁'; // Default folder icon
      let iconType = 'emoji';
      
      if (savedIconData) {
        // Use the saved icon if available
        icon = savedIconData.icon || '📁';
        iconType = savedIconData.iconType || 'emoji';
        console.log(`Processing category ${category.id} (${category.name}), found icon: ${icon}, type: ${iconType}`);
      } else {
        console.log(`No icon data found for category ${category.id} (${category.name}), using default`);
      }
      
      // Process children recursively if they exist
      const children = category.children ? processCategoryTreeIcons(category.children) : [];
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      // Transform the data and add UI-only fields
      const transformedData = data.map(category => {
        ...
        if (savedIconData) {
          // Use the saved icon if available
          icon = savedIconData.icon;
          iconType = savedIconData.iconType;
        } else {
          // Generate a default icon based on first letter if no saved icon
          const defaultIcons = {
          ...
          const firstLetter = category.name.charAt(0).toLowerCase();
          icon = defaultIcons[firstLetter] || '📁';
          
          // Save this default icon to ensure consistency
          const updatedIcons = {...categoryIcons};
          updatedIcons[category.id] = {
            icon: icon,
            iconType: 'emoji'
          };
        ...
        return {
          ...category,
          icon, // Add the icon from localStorage or default
          iconType, // Store the icon type
  ...
  const openEditModal = (category) => {
    ...
    setFormData({
      ...
      icon: categoryIcons[category.id]?.icon || '',
      iconType: categoryIcons[category.id]?.iconType || 'emoji',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modalMode === 'add') {
        ...
        // Always save icon data to localStorage, even if empty
        const updatedIcons = { ...categoryIcons };
        updatedIcons[newCategory.id] = {
          icon: formData.icon || '',
        ...
        const updatedCategory = await categoryService.updateCategory(currentCategory.id, dbFormData);
        if (!updatedCategory) {
        ...
        // Always save icon data to localStorage, even if empty
        const updatedIcons = { ...categoryIcons };
        updatedIcons[currentCategory.id] = {
          icon: formData.icon || '',
          iconType: formData.iconType || 'emoji'
        };
        setCategoryIcons(updatedIcons);
        ...
        const updatedCategoryWithIcon = {
          ...updatedCategory,
          icon: formData.icon || '',
          iconType: formData.iconType || 'emoji'
        };
        
        console.log('Updating category with icon data:', {
        ...
        setCategories(categories.map(category => 
          category.id === currentCategory.id ? updatedCategoryWithIcon : category
        ));
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex space-x-2">
            ...
            <button
              ...
              disabled={loading}
            >
              <RiRefreshLine className={`mr-1 ${loading ? 'animate-spin' : ''}`} />
              Refresh
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            <div className="text-sm text-gray-500 mb-2">
              {filteredAndSortedCategories.length} categories found {viewMode === 'grouped' && currentParentId && '(filtered by parent)'}
            </div>
            ...
            <div className="flex space-x-1 mb-2 md:mb-0">
              <button
                ...
                className={`px-3 py-1 rounded-md text-xs md:text-sm ${viewMode === 'flat' ? 'bg-primary-100 text-primary-700' : 'bg-gray-100 text-gray-700'}`}
              >
                Grid View
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-md text-xs md:text-sm ${viewMode === 'tree' ? 'bg-primary-100 text-primary-700' : 'bg-gray-100 text-gray-700'}`}
              >
                Tree View
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-md text-xs md:text-sm ${viewMode === 'grouped' ? 'bg-primary-100 text-primary-700' : 'bg-gray-100 text-gray-700'}`}
              >
                Grouped View
              </button>
            </div>
      ...
      {loading ? (
        ...
        <div className="card-glass p-4">
          <div className="mb-4 pb-3 border-b border-gray-200">
            <h3 className="text-lg font-medium text-gray-800">Category Hierarchy</h3>
            <p className="text-sm text-gray-500">Showing full category structure with parent-child relationships</p>
          </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-lg font-medium text-gray-800">
                  {currentParentId ? 
                    `Subcategories of ${categories.find(c => c.id === currentParentId)?.name || 'Selected Category'}` : 
                    'Main Categories'}
                </h3>
                        ...
                        <div>
                          ...
                          {!currentParentId && !category.parent_id && 
                            categories.filter(c => c.parent_id === category.id).length > 0 && (
                              ...
                              >
                                View {categories.filter(c => c.parent_id === category.id).length} subcategories
                              </button>
                            )
                          }
                        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredAndSortedCategories.map((category) => (
            <div key={category.id} className="card-glass hover:shadow-lg transition-shadow duration-300">
              ...
              <div className="mt-4 flex justify-between items-center text-sm text-gray-500">
                <div>
                  <span className="font-medium">{category.listingsCount}</span> listings
                </div>
                    ...
                    <div className="flex flex-col space-y-3">
                      ...
                      <div className="flex items-center mb-2">
                        ...
                        <div>
                          <div className="text-sm font-medium text-gray-700">
                            {formData.icon ? 
                              (formData.iconType === 'image' ? 'Custom Image Icon' : 'Emoji Icon') 
                              : 'No icon selected'}
                          </div>
                      ...
                      <div className="flex space-x-2">
                        ...
                        <label className="flex-1 p-2 bg-gray-100 rounded-md text-gray-700 hover:bg-gray-200 flex items-center justify-center cursor-pointer transition-colors">
                          <RiImageAddLine className="w-5 h-5 mr-2" /> Upload Image
                          <input 
                            type="file" 
                            name="iconImage" 
                            className="hidden" 
                      ...
                      {formData.icon && (
                        <button
                          ...
                          onClick={() => setFormData({...formData, icon: '', iconType: 'emoji'})}
                        >
                  ...
                  <div className="mb-4">
                    ...
                    >
                      ...
                      {parentCategories.map(parent => (
                        ...
                        currentCategory && currentCategory.id === parent.id ? null : (
